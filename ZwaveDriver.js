...
class ZwaveDriver extends events.EventEmitter {
	...
	constructor(driverId, options) {
        ...
        this.libdebug = true;    //  to set debug from lib on or off debug is for property zwave driver
    ...
    init(devicesData, callback) {
        ...
        util.log('init devices data', devicesData);
        ...
        devicesData.forEach((deviceData) => {
            ...
            if (this.devicesTokenIdMap) {

                util.log(`  devicesTokenIdMap zwavedriver `, this.devicesTokenIdMap)
                util.log(` deviceData.token  zwavedriver `, deviceData.token)
                util.log(` deviceData.token  zwavedriver type `, typeof  deviceData.token)
                if (this.devicesTokenIdMap[deviceData.token] == null) {

                    this.getSettings(deviceData,  (err, settings) =>{
                        util.log('  zwavedriver  settings 175 ', util.inspect(settings, false, null));
                        util.log(` settings.zw_node_id  `, settings.zw_node_id  )
                        this.devicesTokenIdMap[deviceData.token] = settings.zw_node_id
                        util.log(`  devicesTokenIdMap zwavedriver        == null   `, this.devicesTokenIdMap  )
            ...
            util.log('init device data', deviceData);
	...
	deleted(deviceData, callback) {
        ...
        this.getSettings(deviceData, (err, settings) => {
            util.log('  zwavedriver  settings 175 ', util.inspect(settings, false, null));
            util.log(` settings.zw_node_id  `, settings.zw_node_id)
           delete this.devicesTokenIdMap[deviceData.token] 
            util.log(`  devicesTokenIdMap zwavedriver        == null   `, this.devicesTokenIdMap)
